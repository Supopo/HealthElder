...
import com.xaqinren.healthyelders.databinding.ItemSearchTextMenuBinding;

import java.util.List;


public class MenuTextTagAdapter extends BaseQuickAdapter<SlideBarBean.MenuInfoListDTO, BaseViewHolder> implements LoadMoreModule {
    public MenuTextTagAdapter(int layoutResId) {
        super(layoutResId);
    }
    ...
    protected void convert(BaseViewHolder helper, SlideBarBean.MenuInfoListDTO item) {
        ...
        ItemSearchTextMenuBinding binding = DataBindingUtil.bind(helper.itemView);
        binding.setViewModel(item);
        binding.executePendingBindings();
        TextView tvMenu = helper.getView(R.id.tv_title);
        tvMenu.setTextColor(android.graphics.Color.parseColor(item.getFontColor()));
        helper.setBackgroundColor(R.id.tv_title, android.graphics.Color.parseColor(item.getBackgroundColor()));
    }
