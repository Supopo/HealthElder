...
public class VideoListViewModel extends BaseViewModel {
    ...
    public MutableLiveData<BaseListRes<List<DZVideoInfo>>> dzDatas = new MutableLiveData<>();
    ...
    public void getVideoData(int page, VideoListBean videoListBean,String userId) {
        ...
        if (type == 1) {
            ...
            UserRepository.getInstance().getMyVideoList(dismissDialog, datas, page,  "", userId);
        } else if (type == 4) {
            UserRepository.getInstance().getMyVideoList(dismissDialog, datas, page,  "PRIVATE", userId);
        } else if (type == 5) {
