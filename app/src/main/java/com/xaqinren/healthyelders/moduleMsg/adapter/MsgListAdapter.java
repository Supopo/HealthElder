package com.xaqinren.healthyelders.moduleMsg.adapter;

import androidx.databinding.DataBindingUtil;

import com.chad.library.adapter.base.BaseQuickAdapter;
import com.chad.library.adapter.base.module.LoadMoreModule;
import com.chad.library.adapter.base.viewholder.BaseViewHolder;
import com.xaqinren.healthyelders.R;
import com.xaqinren.healthyelders.databinding.ItemMsgBinding;
import com.xaqinren.healthyelders.moduleMsg.bean.MsgBean;

import java.util.List;

/**
 * 消息列表
 */
public class MsgListAdapter extends BaseQuickAdapter<MsgBean, BaseViewHolder> implements LoadMoreModule {
    public MsgListAdapter(int layoutResId) {
        super(layoutResId);
    }

    @Override
    protected void convert(BaseViewHolder helper, MsgBean item) {
        ItemMsgBinding binding = DataBindingUtil.bind(helper.itemView);
        binding.setViewModel(item);
        binding.executePendingBindings();
        if (helper.getLayoutPosition() == 0) {
            helper.setGone(R.id.line, true);
        }else {
            helper.setGone(R.id.line, false);
        }
    }

    //局部刷新用的
    @Override
    protected void convert(BaseViewHolder helper, MsgBean item, List<?> payloads) {
        super.convert(helper, item, payloads);
        if (payloads.size() > 0 && payloads.get(0) instanceof Integer) {
            //不为空，即调用notifyItemChanged(position,payloads)后执行的，可以在这里获取payloads中的数据进行局部刷新
            int type = (Integer) payloads.get(0);// 刷新哪个部分 标志位
            if (type == 99) {

            }
        }
    }
}
