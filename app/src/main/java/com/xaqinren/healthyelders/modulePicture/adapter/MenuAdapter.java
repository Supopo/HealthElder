...
public class MenuAdapter  extends SwipeMenuAdapter<MenuAdapter.DefaultViewHolder> {
    ...
    public View onCreateContentView(@NonNull ViewGroup parent, int viewType) {
        return LayoutInflater.from(parent.getContext()).inflate(R.layout.item_phpto_sel_nomrl, parent, false);
    }

    @NonNull
    @Override
    public MenuAdapter.DefaultViewHolder onCompatCreateViewHolder(@NonNull View realContentView, int viewType) {
        return new MenuAdapter.DefaultViewHolder(realContentView);
    }
    ...
    public void onBindViewHolder(@NonNull MenuAdapter.DefaultViewHolder holder, int position) {
        ...
        Glide.with(mContext)
                .load(fileInfo.getFileUri())
                .apply(new RequestOptions()
                        .transforms(new CenterCrop(), new RoundedCorners((int) mContext.getResources().getDimension(R.dimen.dp_2))
                        ))
                .into(holder.ivThumb);
    }
    ...
    static class DefaultViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public DefaultViewHolder(@NonNull View itemView) {
            super(itemView);
            ivThumb = (ImageView) itemView.findViewById(R.id.item_iv);

            tvDuration = (TextView) itemView.findViewById(R.id.tv_duration);
            ivDelete = (ImageView) itemView.findViewById(R.id.item_close);
