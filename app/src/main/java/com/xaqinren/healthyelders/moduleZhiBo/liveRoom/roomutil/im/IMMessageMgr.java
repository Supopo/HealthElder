package com.xaqinren.healthyelders.moduleZhiBo.liveRoom.roomutil.im;
...
public class IMMessageMgr implements TIMMessageListener {
    ...
    private String mGroupID;//注意 解散群 退群时候不用存id！！！
                        ...
                        try {
                            ...
                            if (commonJson.cmd != null) {
                                if (commonJson.cmd.equalsIgnoreCase("CustomTextMsg")) {
                                    ...
                                    mMessageListener.onGroupCustomMessage(mGroupID, message.getSender(), data);
                                } else if (commonJson.cmd.equalsIgnoreCase("notifyPusherChange")) {
                                    mMessageListener.onPusherChanged();
                                } else if (commonJson.cmd.equalsIgnoreCase("CustomProductMSg")) {
                                    mMessageListener.onGroupCustomMessage(mGroupID, message.getSender(), data);
                                } else if (commonJson.cmd.equals("C2CMsg")) {
                                    //此处的私发其实还是群发
                                    //判断是不是自己的消息
    ...
    //重新登录的方法，自动登录方法已弃用
    public void autoLogin(String userId, String userSig, final Callback cb) {
    ...
    private static final class DataJson {
        String cmd;
        String msg;
    }
}
